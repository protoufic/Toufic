// Canonical 52-entry race and proof database
...
// Computed metrics
const verifiedStatuses: ProofStatus[] = ["Verified", "Verified placing", "Proof reviewed", "Tracked"];

export const raceMetrics = {
  get totalEntries() {
    return races.length;
  },
  get entriesWithLinks() {
    return races.filter(race => race.links.length > 0).length;
  },
  get verifiedEntries() {
    return races.filter(race => verifiedStatuses.includes(race.status)).length;
  },
  get verifiedPodiums() {
    return races.filter(race => race.podium && verifiedStatuses.includes(race.status)).length;
  },
  get countries() {
    return [...new Set(races.map(race => race.country))].length;
  },
  get countryList() {
    return [...new Set(races.map(race => race.country))];
  },
  get years() {
    return [...new Set(races.map(race => race.year))].sort((a, b) => b - a);
  },
  get disciplines() {
    return [...new Set(races.map(race => race.discipline))];
  }
};

// Personal bests
export const personalBests = {
  fiveK: { time: "19:48", race: "Allure Bleue", result: "2nd overall", link: "https://www.strava.com/activities/17472316459" },
  tenK: { time: "39:42", race: "Envolez-Vous", result: "3rd U23, 23rd overall", link: "https://www.strava.com/activities/17249976869" },
  halfMarathon: { time: "1:29:26", race: "Montpellier Run Festival", result: "9th U23, 140/4,516", link: "https://www.strava.com/activities/18175504201" },
  marathon: { time: "3:42:37", race: "Prague Marathon", result: "Left leg taped", link: "https://www.strava.com/activities/18717374673" },
  ironman703: { time: "6:08:15", race: "IRONMAN 70.3 Warsaw", result: "113th M18-24", link: "https://www.ironman.com/races/im703-warsaw/results" }
};
